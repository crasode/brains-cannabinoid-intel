...
export default async function ResearcherPage({ params }: { params: Promise<{ name: string }> }) {
  const { name } = await params;
  const decoded = decodeURIComponent(name);
  const trials = (await getAllTrials()).filter((trial) => trial.leadResearchers.includes(decoded));
  ...
  return (
    <main className="min-h-screen bg-[#f4efe7] px-6 py-12 text-[#1d2430] md:px-10">
      <div className="mx-auto max-w-6xl">
        <TopNav />
        <p className="text-xs font-semibold uppercase tracking-[0.35em] text-[#c8a96e]">Researcher Intelligence</p>
        <h1 className="mt-3 text-4xl font-[family-name:var(--font-serif)] text-[#0d2035] md:text-5xl">{decoded}</h1>
        <p className="mt-4 max-w-4xl text-sm leading-7 text-[#526174]">{intel.summary}</p>
        <div className="mt-8 grid gap-4 md:grid-cols-4">
          <div className="border border-black/5 bg-white p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#7a8794]">Linked studies</div><div className="mt-2 text-4xl font-[family-name:var(--font-serif)] text-[#0d2035]">{intel.active}</div></div>
