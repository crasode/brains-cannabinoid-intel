...
export default async function SponsorPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  const intel = summarizeEntity(trials, "sponsor", decoded);
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="border border-black/5 bg-white p-6">
            ...
            <div className="mt-5 space-y-4 text-sm leading-7 text-[#526174]">
              ...
              <div><span className="font-medium text-[#0d2035]">Commercial interpretation:</span> {intel.avgScore >= 70 ? 'High-priority sponsor cluster' : intel.avgScore >= 55 ? 'Mid-priority sponsor cluster' : 'Lower-priority sponsor cluster at current signal level'}.</div>
            </div>
          </div>
        </div>
      </div>
