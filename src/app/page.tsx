    ...
    <main className="min-h-screen bg-[#f4efe7] text-[#1d2430]">
      ...
      <section className="px-8 pb-10 md:px-14">
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <Reveal><div className="border border-black/5 bg-white p-6 md:p-8"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#c8a96e]">Highest-value opportunities</p><h2 className="mt-3 text-3xl font-[family-name:var(--font-serif)] text-[#0d2035]">Top scored studies</h2></div><a href="/explorer" className="text-sm font-medium text-[#0d2035] underline underline-offset-4">Open explorer</a></div><div className="mt-8 space-y-4">{data.topTrials.map((trial) => (<div key={trial.id} className="border border-black/6 bg-[#fcfaf7] p-5"><div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div className="max-w-3xl"><div className="text-xs uppercase tracking-[0.18em] text-[#7a8794]">{trial.id} · {trial.status} · {trial.phase}</div><h3 className="mt-2 text-2xl font-[family-name:var(--font-serif)] text-[#0d2035]">{trial.title}</h3><p className="mt-3 text-sm leading-7 text-[#526174]"><span className="font-medium text-[#0d2035]">Lead:</span> {trial.leadResearchers[0] || "Not listed"}{trial.institutions[0] ? ` · ${trial.institutions[0]}` : ""}</p><p className="mt-1 text-sm leading-7 text-[#526174]"><span className="font-medium text-[#0d2035]">Sponsor:</span> {trial.sponsors.join(", ") || "Not listed"}</p><p className="mt-1 text-sm leading-7 text-[#526174]"><span className="font-medium text-[#0d2035]">Countries:</span> {trial.countries.join(", ") || "Not listed"}</p></div><div className={`min-w-[130px] px-4 py-3 text-center ${scoreTone(trial.commercialScore)}`}><div className="text-xs uppercase tracking-[0.18em]">Score</div><div className="mt-1 text-4xl font-[family-name:var(--font-serif)]">{trial.commercialScore}</div></div></div><div className="mt-4 grid gap-3 text-sm text-[#526174] md:grid-cols-3"><div>Patents: <span className="font-semibold text-[#0d2035]">{trial.patentCount}</span></div><div>Grants: <span className="font-semibold text-[#0d2035]">{trial.grantCount}</span></div><div>Publications: <span className="font-semibold text-[#0d2035]">{trial.publicationCount}</span></div></div></div>))}</div></div></Reveal>
