    ...
    <div className="border border-black/6 bg-[#fcfaf7] p-5">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="max-w-3xl">
          ...
          <p className="mt-3 text-sm leading-7 text-[#526174]"><span className="font-medium text-[#0d2035]">Lead:</span> {trial.leadResearchers[0] ? <Link href={`/researcher/${encodeURIComponent(trial.leadResearchers[0])}`} className="underline underline-offset-4">{trial.leadResearchers[0]}</Link> : "Not listed"}{trial.institutions[0] ? ` · ` : ""}{trial.institutions[0] ? <Link href={`/institution/${encodeURIComponent(trial.institutions[0])}`} className="underline underline-offset-4">{trial.institutions[0]}</Link> : null}</p>
          ...
          <p className="mt-1 text-sm leading-7 text-[#526174]"><span className="font-medium text-[#0d2035]">Why it matters:</span> {summarizeWhyItMatters(trial)}.</p>
        </div>
